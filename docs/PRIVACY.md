...
| Conservation              | Durée de vie du projet ; purge idempotente par le plan de contrôle lors de sa suppression définitive |
| Sécurité                  | JWT Ed25519 court, isolation SQL par workspace, TLS au reverse proxy, données absentes des logs      |
| Droits                    | Export ICS/CSV disponible ; rectification/suppression via API ; purge complète du workspace          |
