...
describe("deriveTheme — WCAG guarantees", () => {
  it("primary ink ≥ AAA (7:1) and secondary ink ≥ AA (4.5:1) on surface, both modes", () => {
    for (const mode of modes) {
      for (const [base, accent] of pairs) {
        ...
        expect(
          contrastRatio(parseHex(t["--cal-ink"]), surface),
        ).toBeGreaterThanOrEqual(6.9);
        expect(
          contrastRatio(parseHex(t["--cal-ink-700"]), surface),
        ).toBeGreaterThanOrEqual(4.4);
        expect(
          contrastRatio(parseHex(t["--cal-ink-muted"]), surface),
        ).toBeGreaterThanOrEqual(4.4);
      }
    }
  });
  it("on-accent ink prefers white and clears large-text AA (3:1) on the accent", () => {
    ...
    for (const mode of modes) {
      for (const [base, accent] of pairs) {
        ...
        expect(
          contrastRatio(
            parseHex(t["--cal-accent-ink"]),
            parseHex(t["--cal-accent"]),
          ),
  ...
  it("honours an explicit accentInk override", () => {
    ...
    expect(
      deriveTheme("#ffffff", "#0d9488", "light", {}, "not-a-color")[
        "--cal-accent-ink"
      ],
    ).toBe("#ffffff");
  });
...
describe("deriveTheme — invalid base/accent", () => {
  it("throws on invalid base or accent hex", () => {
    expect(() => deriveTheme("nope", "#3b82f6", "light")).toThrowError(
      /Invalid hex/,
    );
    expect(() => deriveTheme("#ffffff", "xyz", "light")).toThrowError(
      /Invalid hex/,
    );
  });
});
