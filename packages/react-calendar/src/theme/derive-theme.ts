...
const onAccentInk = (accent: Rgb): Rgb => {
  ...
  return contrastRatio(WHITE, accent) >= contrastRatio(BLACK, accent)
    ? WHITE
    : BLACK;
};
  ...
  if (eventColors) {
    for (const [rawKey, hex] of Object.entries(eventColors)) {
      ...
      try {
        ...
        console.warn(
          `[react-calendar] invalid event colour for "${rawKey}": ${hex}`,
        );
        continue;
      }
      ...
      tokens[`--cal-event-${key}-ink`] = formatHex(
        ensureContrastAA(WHITE, chip, AA),
      );
      tokens[`--cal-event-${key}-soft`] = formatHex(soft);
      tokens[`--cal-event-${key}-soft-ink`] = formatHex(
        ensureContrastAA(chip, soft, AA),
      );
    }
  }
