 ...
 */
export function CalTimeGridView<TMeta = unknown>(
  props: CalTimeGridViewProps<TMeta>,
): ReactNode {
  ...
  const previewGeometry = (
    ...
    return {
      startOffset: (cs - vm.dayStartMinutes) / total,
      span: (ce - cs) / total,
    };
  };
  ...
  const eventStyle = (
    ...
    const key =
      ev.event.status !== undefined ? sanitizeStatusKey(ev.event.status) : "";
    const bg =
      key !== ""
        ? `var(--cal-event-${key}, var(--cal-accent))`
        : "var(--cal-accent)";
    const fg =
      key !== ""
  ...
  const onEventPointerDown = (
    ...
    if (
      !editable ||
      ev.event.isReadonly === true ||
      ev.event.editable === false
    ) {
      return;
    }
    ...
    const colEl = (dom.currentTarget as HTMLElement).closest<HTMLElement>(
      ".cal-tg__col",
    );
    if (colEl === null) {
      return;
    ...
    const total = vm.dayEndMinutes - vm.dayStartMinutes;
    const pxPerMinute =
      axisSize(colEl.getBoundingClientRect()) / Math.max(1, total);
    const start = adapter.toZoned(ev.event.start, resolvedZone);
    const end =
      ev.event.end === undefined
        ? start
        : adapter.toZoned(ev.event.end, resolvedZone);
  ...
  const announceGesture = (
    ...
    if (kind === "resize-end") {
      announce(
        a11y.resizedLabel(
          zonedFromMs(drag.originEndMs + deltaMinutes * 60_000),
        ),
      );
    } else {
      announce(
        a11y.movedLabel(
          zonedFromMs(drag.originStartMs + deltaMinutes * 60_000),
        ),
      );
    }
  };
  ...
  const onEventKeydown = (
    ...
    const grabbing =
      drag !== null &&
      drag.pointerId === KEYBOARD_POINTER &&
      drag.eventId === ev.event.id;
  ...
  const isEditing = (ev: PositionedEvent<TMeta>): boolean =>
    editingId === ev.event.id;
  ...
  const onInlineKeydown = (
    ev: PositionedEvent<TMeta>,
    dom: KeyboardEvent<HTMLInputElement>,
  ): void => {
    if (dom.key === "Enter") {
      dom.preventDefault();
  ...
  const chipStyle = (chip: PositionedChip<TMeta>): CSSProperties => {
    const key =
      chip.event.status !== undefined
        ? sanitizeStatusKey(chip.event.status)
        : "";
    const bg =
      key !== ""
  ...
  const onColumnPointerDown = (
    ...
    const frac =
      (axisClient(dom) - axisStart(rect)) / Math.max(1, axisSize(rect));
    const minutes = vm.dayStartMinutes + Math.max(0, Math.min(1, frac)) * total;
    const anchorMs = adapter.addMinutes(
      column.date,
      Math.round(minutes),
    ).epochMs;
          ...
          {vm.ticks
            ...
            .map((tick) => (
              <div
                ...
                style={
                  { "--tick-pos": `${tick.offset * 100}%` } as CSSProperties
                }
              >
          ...
          {vm.columns.map((column) => {
            ...
            return (
              ...
              >
                ...
                {ghost !== null && (
                  <div
                    className="cal-tg__create-ghost"
                    style={ghost}
                    aria-hidden="true"
                  ></div>
                )}
                ...
                {column.events.map((ev) => (
                  <button
                    ...
                    className={[
                      "cal-tg__event",
                      ev.continuesBefore
                        ? "cal-tg__event--continues-before"
                        : "",
                  ...
                  >
                    {isEditing(ev) ? (
                      ...
                      <span className="cal-tg__event-title">
                        {ev.event.title}
                      </span>
                    )}
                    ...
                    {editable && ev.event.isReadonly !== true && (
                      <>
                        <span
                          ...
                          aria-hidden="true"
                          onPointerDown={(e) =>
                            onEventPointerDown(ev, "resize-start", e)
                          }
                          onPointerMove={onEventPointerMove}
                          onPointerUp={(e) => onEventPointerUp(ev, e)}
                          onPointerCancel={onEventPointerCancel}
                        ...
                        <span
                          ...
                          aria-hidden="true"
                          onPointerDown={(e) =>
                            onEventPointerDown(ev, "resize-end", e)
                          }
                          onPointerMove={onEventPointerMove}
                          onPointerUp={(e) => onEventPointerUp(ev, e)}
                          onPointerCancel={onEventPointerCancel}
                ...
                {column.nowOffset !== null && (
                  <div
                    className="cal-tg__now"
                    style={nowStyle(column.nowOffset)}
                    aria-hidden="true"
                  >
