...
describe("CalAgendaView", () => {
  it("renders a day heading and an event row with time + title", () => {
    ...
    expect(el.querySelector(".cal-agenda__date")?.textContent).toContain(
      "June",
    );
  ...
  it('shows "All day" for all-day events', () => {
    ...
    expect(el.querySelector(".cal-agenda__time")?.textContent).toContain(
      "All day",
    );
  });
...
describe("CalAgendaView — recurrence", () => {
  it("expands a recurring event into the agenda window", () => {
    ...
    const rows = [...container.querySelectorAll(".cal-agenda__row")].filter(
      (r) => r.textContent?.includes("Weekly sync"),
    );
