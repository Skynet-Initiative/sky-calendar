...
describe("CalRecurrenceEditor", () => {
  it("reflects an existing weekly rule (weekday chips, frequency)", () => {
    ...
    expect(el.querySelector<HTMLSelectElement>("#cal-rec-freq")?.value).toBe(
      "weekly",
    );
  ...
  it("changing the count end-type writes COUNT", () => {
    let rule = "FREQ=DAILY";
    const el = render(rule, (r) => (rule = r));
    const countRadio = el.querySelectorAll<HTMLInputElement>(
      'input[type="radio"]',
    )[1];
  ...
  it("shows a live RRULE preview", () => {
    ...
    expect(el.querySelector(".cal-rec__preview")?.textContent).toContain(
      "FREQ=MONTHLY",
    );
  });
});
