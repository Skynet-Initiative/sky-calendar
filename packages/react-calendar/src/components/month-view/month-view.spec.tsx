...
describe("CalMonthView", () => {
  it('opens a "+N more" popover listing every event on the day', () => {
    ...
    const titles = [...el.querySelectorAll(".cal-more__title")].map((t) =>
      t.textContent?.trim(),
    );
...
describe("CalMonthView — keyboard navigation", () => {
  it("moves roving focus with arrow keys and selects with Enter", () => {
    ...
    const focusedBefore = el.querySelector(
      '[role="gridcell"][tabindex="0"]',
    ) as HTMLElement;
    // ArrowRight → next day
    fireEvent.keyDown(grid, { key: "ArrowRight" });
    const afterRight = el.querySelector(
      '[role="gridcell"][tabindex="0"]',
    ) as HTMLElement;
    expect(afterRight.dataset["epoch"]).not.toBe(
      focusedBefore.dataset["epoch"],
    );
    // ArrowDown → +7 days (one week)
    fireEvent.keyDown(grid, { key: "ArrowDown" });
    const afterDown = el.querySelector(
      '[role="gridcell"][tabindex="0"]',
    ) as HTMLElement;
...
describe("CalMonthView — recurrence", () => {
  it("expands a recurring event into multiple chips across the month", () => {
    const { container } = rtlRender(
      <CalendarProvider
        dateAdapter={adapter}
        recurrenceAdapter={new RruleRecurrenceAdapter()}
      >
        <CalMonthView
          events={[
            {
    ...
    const chips = [...container.querySelectorAll(".cal-chip__title")].filter(
      (c) => c.textContent?.includes("Weekly sync"),
    );
