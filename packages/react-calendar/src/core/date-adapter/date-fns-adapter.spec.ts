...
const at = (iso: string, zone: string): ZonedDateTime =>
  a.toZoned(new Date(iso), zone);
...
describe("DateFnsDateAdapter — queries", () => {
  ...
  it("isSameDay compares the local calendar day", () => {
    ...
    expect(
      a.isSameDay(ny, a.toZoned(new Date("2026-06-16T02:00:00Z"), NY)),
    ).toBe(true);
...
describe("DateFnsDateAdapter — calendar systems (getEra)", () => {
  ...
  it("gregory returns the civil y/m/d", () => {
    expect(a.getEra(d, "gregory")).toMatchObject({
      year: 2026,
      month: 6,
      day: 15,
    });
...
describe("DateFnsDateAdapter — format", () => {
  ...
  it("throws on an unsupported pattern", () => {
    expect(() => a.format(d, "nonsense", "en-US")).toThrowError(
      /Unsupported format pattern/,
    );
  });
});
