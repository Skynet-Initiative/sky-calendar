...
export function buildTimelineView<TMeta = unknown>(
  ...
  const lastDayBase = adapter.startOfDay(
    adapter.addDays(args.viewDate, args.days - 1),
  );
  ...
  const minOf = (d: ZonedDateTime): number =>
    adapter.differenceInMinutes(d, rangeStart);
  ...
  const resourceRows: ResourceRow<TMeta>[] = flat.map(
    ({ resource, depth, hasChildren }) => {
      ...
      const offBands = buildOffHoursShade(
        adapter,
        resource.workHours,
        args,
        base,
...
function startOfUnit(
  ...
  switch (unit) {
    ...
    case "hour": {
      ...
      return adapter.addMinutes(
        adapter.startOfDay(d),
        Math.floor(into / 60) * 60,
      );
    }
    case "minute": {
      const into = adapter.getMinutesIntoDay(d);
