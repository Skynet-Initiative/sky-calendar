...
const allDays = (vm: ReturnType<typeof buildMonthView>) =>
  vm.weeks.flatMap((w) => w.days);
...
describe("buildMonthView — grid", () => {
  ...
  it("period spans the grid (start = first cell, end exclusive)", () => {
    ...
    expect(vm.period.end.epochMs).toBe(
      adapter.startOfDay(adapter.addDays(lastDay, 1)).epochMs,
    );
  });
});
...
describe("buildMonthView — multi-day chip packing", () => {
  ...
  it("a single-day timed event covers exactly its day (span 1)", () => {
    ...
    const chip = dayLabeled(vm1, "Jun 15")!.events.find(
      (c) => c.event.id === "m",
    )!;
  ...
  it("an event ending exactly at midnight does not cover the next day (half-open)", () => {
    ...
    expect(
      dayLabeled(vm1, "Jun 16")!.events.some((c) => c.event.id === "allday"),
    ).toBe(false);
  });
});
