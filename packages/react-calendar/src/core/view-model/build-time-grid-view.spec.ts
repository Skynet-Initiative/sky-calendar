...
describe("buildTimeGridView — timed placement", () => {
  ...
  it("clips an event to the visible window and flags continuation", () => {
    const vm = buildTimeGridView(adapter, {
      ...
      events: [
        {
          id: "a",
          start: at("2026-06-15T11:00:00Z"),
          end: at("2026-06-15T13:00:00Z"),
        },
      ],
      days: 1,
      anchorToWeek: false,
      ...
      events: [
        {
          id: "a",
          start: at("2026-06-15T13:00:00Z"),
          end: at("2026-06-15T15:00:00Z"),
        },
        {
          id: "b",
          start: at("2026-06-15T14:00:00Z"),
          end: at("2026-06-15T16:00:00Z"),
        },
      ],
      days: 1,
      anchorToWeek: false,
  ...
  it("horizontal stacks near-adjacent short events onto separate lanes (min visual width)", () => {
    ...
    expect(vertical.columns[0]!.events.every((e) => e.laneCount === 1)).toBe(
      true,
    );

    // Horizontal: packed as ≥57 min each → stacked onto two lanes, no overlap.
    const horizontal = buildTimeGridView(adapter, {
      ...shared,
  ...
  it("a multi-day timed event spans the band across columns", () => {
    const vm = buildTimeGridView(adapter, {
      ...
      events: [
        {
          id: "a",
          start: at("2026-06-16T13:00:00Z"),
          end: at("2026-06-18T14:00:00Z"),
        },
      ],
      days: 7,
      anchorToWeek: true,
  ...
  it("handles a spring-forward day window without error", () => {
    const vm = buildTimeGridView(adapter, {
      ...
      events: [
        {
          id: "a",
          start: at("2026-03-08T12:00:00Z"),
          end: at("2026-03-08T13:00:00Z"),
        },
      ],
      days: 1,
      anchorToWeek: false,
...
describe("buildTimeGridView — hour12", () => {
  ...
  const tickLabel = (
    vm: ReturnType<typeof buildTimeGridView>,
    min: number,
  ): string | undefined => vm.ticks.find((t) => t.minutes === min)?.label;
