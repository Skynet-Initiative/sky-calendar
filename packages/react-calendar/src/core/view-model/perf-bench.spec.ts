...
function makeEvents(count: number, dayBaseIso: string): CalendarEvent[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const start =
      baseMs + (i % 20) * 30 * 60_000 + Math.floor(i / 20) * 86_400_000;
    return {
      id: `e${i}`,
      title: `Event ${i}`,
...
describe.runIf(process.env.RUN_PERF_BENCHMARKS === "1")(
  "perf budgets (run on an unloaded benchmark host)",
  () => {
    it("buildMonthView with 500 events stays well under budget", () => {
      const events = makeEvents(500, "2026-06-01T12:00:00Z");
      const ms = bestOf(5, () =>
        buildMonthView(adapter, {
    ...
    it("buildTimelineView (100 resources × 2000 events) stays under budget", () => {
      const resources: CalendarResource[] = Array.from(
        { length: 100 },
        (_, i) => ({
          id: `r${i}`,
          name: `Resource ${i}`,
